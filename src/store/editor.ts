...
export const useEditor = defineStore("editor", {
  ...
  actions: {
    ...
    updateNodeSlotsProps(nodeUuid: string, slotsProps: Record<string, string[]>) {
      const node = this.nodeMap.get(nodeUuid);
      if (node) {
        node.slotsProps = slotsProps;
      }
    },
    ...
    setNodes(nodes: CraftNode[]) {
      ...
      const addNode = (node: CraftNode) => {
        ...
        const initializedSlots: Record<string, CraftNode[]> = node.slots
          ? {}
          : { default: [] };
        if (node.slots) {
          Object.keys(node.slots).forEach((slotName) => {
            initializedSlots[slotName] = [];
          });
        }

        this.nodeMap.set(node.uuid, { ...node, slots: initializedSlots });
        if (!node.parentUuid) {
    ...
    insertNodeBefore(
      ...
      if (parent && parent.slots) {
        const targetSlotName =
          slotName ||
          Object.keys(parent.slots).find((key) =>
  ...
  getters: {
    ...
    nodeTree: (state): CraftNode[] => {
      const buildTree = (nodeUuid: string): CraftNode => {
        ...
        if (node.slots) {
          Object.entries(node.slots).forEach(([slotName, slotChildren]) => {
            slots[slotName] = slotChildren.map((child) =>
